  ...
  const createRandomCubes=()=>{
    ...
    for (let i = 0; i < randomValues; i++) {
      ...
      cube.style.width = `${size}px`;
        cube.style.backgroundColor = randomColors[Math.floor(Math.random() * randomColors.length)];
  ...
  useEffect(()=>{
    const handleResize = () => {
      ...
      for (let i = 0; i < cubes.length; i++) {
        cubes[i].style.transform = `translate(${Math.random() * 100}vw, ${Math.random() * 100}vh)`;
      }
    };
